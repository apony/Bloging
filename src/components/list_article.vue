<template>
  <div>
    <div class="card"
         ...
         :key="item.id">
      <p class="card_title"
         :id="'anchor-'+item.type">{{item.type}}</p>
      <ul>
        <li v-for="li in item.list"
            :key="li.id">
          <span class="date">{{li.date}}</span>
          <router-link :to="'/detail/'+li._id"
                       class="title">{{ li.title }}</router-link>
